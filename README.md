...
The BizMind AI chat uses a **grounded NLP-to-SQL pipeline** designed for maximum precision:

1. **Query Generation** — Qwen2.5 receives the user's question and the database schema, then writes a targeted `SELECT` SQL query. It uses `LIKE` for case-insensitive matching (e.g., "furniture" matches "Furniture").
2. **Execution** — Python executes the query against the SQLite database. Results are converted into Markdown tables for the AI to read.
3. **Response Generation** — The AI summarizes the database results into a natural language answer. It is strictly grounded to the provided data to prevent hallucinations.

**Key Features:**
- **Rupee Support:** All currency values are automatically formatted with the **₹** symbol.
- **Hallucination Prevention:** If data isn't in the database, the AI will clearly state it cannot find information rather than making up numbers.
- **Audit Logs:** Every query generated by the AI is logged in `bizmind.log` for transparency and auditing.
